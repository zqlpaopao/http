...
![image-20210329135558566](geekTime-http.assets/image-20210329135558566.png)
...
![image-20210329140328985](geekTime-http.assets/image-20210329140328985.png)
...
![image-20210329140544718](geekTime-http.assets/image-20210329140544718.png)
...
2. HTTP/2 的消息不再是“Header+Body”的形式，而是分散为多个二进制“帧”；
3. HTTP/2 使用虚拟的“流”传输消息，解决了困扰多年的“队头阻塞”问题，同时实现了“多路复用”，提高连接的利用率；
4. HTTP/2 也增强了安全性，要求至少是 TLS1.2，而且禁用了很多不安全的密码套件。
...
是“化整为零”的思路，但 HTTP/2 数据分帧后“Header+Body”的报文结构就完全消失了，协议看到的只是一个个的“碎片”。

![image-20210406205221046](geekTime-http.assets/image-20210406205221046.png)
...
![image-20210406205926195](geekTime-http.assets/image-20210406205926195.png)
...
对比了 HTTP/1、HTTPS 和 HTTP/2 的协议栈，你可以清晰地看到，HTTP/2 是建立在“HPack”“Stream”“TLS1.2”基础之上的，比 HTTP/1、HTTPS 复杂了一些。

![image-20210406210113593](geekTime-http.assets/image-20210406210113593.png)
...
```
http2 相比http1.x 在header和body部分作出改变
```

![image-20210519172450268](geekTime-http.assets/image-20210519172450268.png)



## 2、帧（frame）、流（stream）、消息（message）示意图

![在这里插入图片描述](geekTime-http.assets/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM4OTM3NjM0,size_16,color_FFFFFF,t_70.png)

上图中展示了 一个tcp连接中 有多个stream流 ，每个流有多个frame，一个流当中的frame是串行的
...
![image-20210519174034352](geekTime-http.assets/image-20210519174034352.png)

Headers Frame：帧头
固定的<font color=red size=5x>**9**</font>个字节,(24+8+8+1+31)/8 = 9)呈现，变化的为帧的负载（Frame Payload），
负载内容是由帧类型决定的（Type）。

Length：帧长度
无符号的自然数，24个比特表示，仅表示帧负载（Frame Payload）所占用的字节数，不包括帧头所占用的9字节，默认大小区间为0～16384（2^14），
一旦超过默认的最大值，发送方将不再允许发送，除非接收方定义的SETTINGS_MAX_FRAME_SIZE（一般此值的区间为2^14-2^24）值的通知。

Type：帧类型
8个比特表示，定义了帧负载的具体格式和帧的语义，HTTP/2规范定义了10个帧类型，这里不包括实现类型和扩展类型

Flags：帧的标志位
8个比特表示，服务于激凸帧类型，默认值为0x0，8个比特可以容纳8个不同的标志，比如，PADDED值为0x8，二进制表示为00001000；END_HEADERS值为0x4，二进制表示为00000100；END_STREAM值为0X1，二进制为00000001。可以同时在一个字节中传达三种标志位，二进制表示为00001101，即0x13。因此，后面的帧结构中，标志位一般会使用8个比特表示，若某位不确定，使用问号?替代，表示此处可能会被设置标志位

R；帧保留比特位
HTTP/2预警下为保留的比特位，固定值位0x0

Stream Identifier ：流标识符
无符号的31比特表示自然数，0x0值表示位帧进作用域连接，不隶属于单独的流，
   1. 已使用的流标识符不能被再次使用；
   2. 新建流的标识符要大于已有流和预留的流的标识符；
   3. 客户端创建的流以奇数表示，服务器端创建流以偶数表示；
   4. 当超过最大数值的时候，会断开连接，重新建立连接
   5. 若是客户端，需要关闭连接，创建新的连接创建新流；
   6. 若是服务器端，需要发送一个GOAWAY帧通知客户端，强迫其打开一个新连接；

   - 0 ~ 2^14（16384）为默认约定长度，所有端点都需要遵守 - 2^14 (16,384) ~ 2^24-1(16,777,215)此区间数值，需要接收方设置SETTINGS_MAX_FRAME_SIZE参数单独赋值 - 一端接收到的帧长度超过设定上限或帧太小，需要发送FRAME_SIZE_ERR错误 - 当帧长错误会影响到整个连接状态时，须以连接错误对待之；比如HEADERS，PUSH_PROMISE，CONTINUATION，SETTINGS，以及帧标识符不该为0的帧等，都需要如此处理 - 任一端都没有义务必须使用完一个帧的所有可用空间 - 大帧可能会导致延迟，针对时间敏感的帧，比如RST_STREAM, WINDOW_UPDATE, PRIORITY，需要快速发送出去，以免延迟导致性能等问题



## 4、Http2的帧类型

![image-20210519180753332](geekTime-http.assets/image-20210519180753332.png)

集体type的含义和类型表示请看

https://blog.csdn.net/makenothing/article/details/53241399
